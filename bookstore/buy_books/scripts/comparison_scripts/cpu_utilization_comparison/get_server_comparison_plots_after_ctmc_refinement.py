...
server_cpu_utilization_path = './bookstore/buy_books/buy_books_final/1/transformed_data/server_level_res_utilization_data'
...
for server in servers:
    ...
    estimated_data_path_container = f"{estimated_data_path}/{server}/refined_cpu_utilization_{server}.csv"
    ...
    # Trim both datasets to the same range
    ...
    mse = mean_squared_error(merged_data['CPU Utilization (%)_actual'], merged_data['refined_cpu'])
    rmse = np.sqrt(mse)
    mae = mean_absolute_error(merged_data['CPU Utilization (%)_actual'], merged_data['refined_cpu'])

    plt.figure(figsize=(10, 6))
    plt.plot(grouped['Time (s)'], grouped['CPU Utilization (%)'], label='Actual CPU Utilization', marker='o')
    plt.plot(estimated_cpu_data['Time (s)'], estimated_cpu_data['refined_cpu'], label='Estimated CPU Utilization', color='red')
    plt.xlabel('Time (s)')
    ...
    plot_file_name = f"{output_dir}/{server}_ctmc_cpu_comparison.png"
