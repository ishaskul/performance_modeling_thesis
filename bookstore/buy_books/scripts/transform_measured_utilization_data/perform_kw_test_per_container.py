    ...
    for server in servers:
        ...
        for json_file_path in json_files:
            if(server == "gl2"):
                ...
                server_nam = server
            output_dir = os.path.join(output_base_dir, server_nam, f"cpu_util_data_per_container")
            ...
            for container_name, records in service_data.items():
                if(server == "gl2"):
                    ...
                    server_name = server
                outputPathForPerContainerCSVs = os.path.join(output_base_dir, server_name, "cpu_util_data_per_container", container_name)
                os.makedirs(outputPathForPerContainerCSVs, exist_ok=True)

                csv_file_path = os.path.join(outputPathForPerContainerCSVs, f"cpu_usage_output_{container_name}_it_{i}.csv")
...
for server in green_lab_machines:
    for container_name in os.listdir(os.path.join(output_base_dir, server, "cpu_util_data_per_container")):
        container_dir = os.path.join(output_base_dir, server, "cpu_util_data_per_container", container_name)
        ...
        if len(cpu_usage_across_iterations) == 3: 
