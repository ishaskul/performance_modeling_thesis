...
input_base_dir = '../../follow_users_final_run'
container_pid_mapping_csv_dir = '../../follow_users_final_run' 
output_base_dir = '../../follow_users_final_run/transformed_data_kw_test'
...
for i in range(1, 11):
    for server in servers:
        ...
        if (server == 'gl2'):
            ...
            server_name = server
        output_dir = os.path.join(output_base_dir, server_name, "power_consumption_data_per_container")
        ...
        for json_file_path in json_files:
            ...
            for service, records in service_data.items():
                ...
                csv_file_path = os.path.join(output_dir, service,f"power_consumption_output_{service}_it_{i}.csv")
            ...
            if nginx_data:
                ...
                os.makedirs(service_output_dir, exist_ok=True)
                nginx_csv_path = os.path.join(output_dir, "nginx", f"power_consumption_output_nginx_it_{i}.csv")
...
for server in servers:
    if (server == "gl2"):
        ...
        server_name = server
    for container_name in os.listdir(os.path.join(output_base_dir, server_name, "power_consumption_data_per_container")):
        container_dir = os.path.join(output_base_dir, server_name, "power_consumption_data_per_container", container_name)
        ...
        if len(power_consumption_across_iterations) == 10:
